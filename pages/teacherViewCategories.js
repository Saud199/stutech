...
class TeacherViewCategories extends Component{
    ...
    constructor() {
      ...
      addTeacherCategories() {
        const {teacherCategoriesArray} = this.state;
        teacherCategoriesArray.push({name : 'SSUET' , image : require('../images/job.jpg')});
        teacherCategoriesArray.push({name : 'Oracle' , image : require('../images/oracle.png')});
        teacherCategoriesArray.push({name : 'Decima' , image : require('../images/decima.png')});
        teacherCategoriesArray.push({name : 'App Bakers' , image : require('../images/ssuet.png')});
      }
    
      render() {
        const {teacherCategoriesArray} = this.state;
        this.addTeacherCategories();
        return (
          ...
          <Container>
            ...
            <Tabs tabBarBackgroundColor="#14c2e0" renderTabBar={()=> <ScrollableTab />} >
              ...
              { teacherCategoriesArray.map((val , ind) => {
                return(
...
export default withNavigation (TeacherViewCategories);